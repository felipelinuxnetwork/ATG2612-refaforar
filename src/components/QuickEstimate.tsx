...
export const QuickEstimate: React.FC = () => {
  ...
  return (
    <section id="calculadora" className="py-24 bg-slate-950 relative overflow-hidden">
      {/* Background Accent */}
      <div className="absolute top-1/2 right-1/4 w-[500px] h-[500px] bg-red-900/10 rounded-full blur-3xl pointer-events-none -z-10" />
          ...
          <form onSubmit={handleWhatsAppSend} className="space-y-8">
                ...
                {services.map((svc) => (
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-xs sm:text-sm font-semibold text-left transition-all border flex items-center justify-between ${
                      serviceType === svc
                        ? 'bg-red-600/20 text-white border-red-500 shadow-sm shadow-red-950'
                        : 'bg-slate-900/80 text-slate-300 border-slate-800 hover:bg-slate-800/80 hover:border-slate-700'
                    }`}
                  >
                    <span>{svc}</span>
                    {serviceType === svc && <Check className="w-4 h-4 text-red-400 shrink-0 ml-1" />}
                  </button>
                ))}
              </div>
            </div>
                ...
                {urgencyOptions.map((urg) => (
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-xs sm:text-sm font-semibold text-center transition-all border ${
                      urgency === urg
                        ? 'bg-red-600/20 text-white border-red-500 shadow-sm'
                        : 'bg-slate-900/80 text-slate-300 border-slate-800 hover:bg-slate-800/80 hover:border-slate-700'
                    }`}
                  >
                    {urg}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-slate-900/90 text-white placeholder-slate-500 rounded-xl border border-slate-800 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 text-sm"
                  />
                </div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-slate-900/90 text-white placeholder-slate-500 rounded-xl border border-slate-800 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 text-sm"
              />
            </div>
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center gap-4">
              <button
                type="submit"
                className="w-full sm:flex-1 py-4 px-6 rounded-xl font-bold text-white bg-gradient-to-r from-red-600 via-red-600 to-red-700 hover:from-red-500 hover:to-red-600 shadow-xl shadow-red-900/40 flex items-center justify-center gap-3 transition-all transform hover:-translate-y-0.5 text-base cursor-pointer"
              >
